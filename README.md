...
```
├── excel/              # Summary analysis: regional & shipping-mode breakdowns, 
│                        # plus a 2,000-row sample of the cleaned data
...
> **Note on the raw dataset:** the full raw dataset (180,519 rows) is not included in this repo due to GitHub's file size limits. It's publicly available on [Kaggle](https://www.kaggle.com/datasets/shashwatwork/dataco-smart-supply-chain-for-big-data-analysis). The `excel/` folder instead contains a summary workbook with the key regional and shipping-mode findings (with live formulas) plus a representative sample of the cleaned data — customer PII (names, emails, passwords, street addresses) has been removed from the sample for privacy.
